---
name: product-b-funnel
display_name: "Conversion Funnel Intelligence | 转化漏斗智能分析"
description: |
  This skill should be used when the user needs to analyze marketing conversion funnels for Product_B (a fintech app) and generate budget allocation recommendations.
  
  It merges AppsFlyer event data with CRM conversion data, calculates channel validity metrics (CRM conversions / AF events × 100%), classifies channels into High/Mid/Low effectiveness tiers, and recommends AF CAP values based on effectiveness tiers.
  
  Key principle: retain all channels (do not pause low-effectiveness channels), only adjust CAP to maintain optimization flexibility.
  
  Applicable for monthly Product_B budget allocation decisions in affiliate marketing operations.
version: 1.0.0
agent_created: true
compatibility:
...
Conversion Funnel Intelligence — Product_B 营销转化漏斗分析与预算规划工具。

适用于每月 Product_B 渠道预算分配决策，核心输出：**各渠道 AF CAP 建议值** + **渠道保留策略**（低有效性渠道只降 CAP，不暂停）。

---

## 适用场景

- 每月 Product_B 渠道预算分配，需计算各渠道有效性并推荐 AF CAP
...
| 指标 | 计算公式 | 说明 |
|------|---------|------|
| AF 事件数 | COUNT(af_events) | AppsFlyer 记录的目标事件数 |
| CRM 转化数 | COUNT(crm_users) | CRM 系统中实际完成注册的用户数 |
...
```
AppsFlyer 关键事件：product_b_core_event
含义：核心转化事件（Product_B 关键指标）
```
...
**重要原则**：低有效性渠道降低 CAP 但不暂停，保留优化空间（Fintech 流量质量波动大）。

---

## 完整执行脚本

> 保存为 `product_b_funnel.py`，与 AF CSV 和 CRM xlsx 放在同一目录运行。
...
"""
product_b_funnel.py
Conversion Funnel Intelligence — Product_B 漏斗分析完整脚本

功能：
1. 加载 AppsFlyer 事件数据（过滤目标事件）
2. 按 AppsFlyer ID 去重（每设备取最早事件）
3. 加载 CRM 转化数据（source_bank + user_count）
4. 按渠道合并，计算有效性（CRM转化数 / AF事件数）
5. 根据有效性分类，计算 Factor 和 AF CAP 建议
6. 输出带颜色标注的 Excel 报告（绿/黄/红）

用法：
    python3 product_b_funnel.py \
        --af product_b_in-app-events_*.csv \
        --crm crm_data.xlsx \
        --output product_b_funnel_result.xlsx
"""
...
# ── 配置区 ─────────────────────────────────────────────
TARGET_EVENT = 'product_b_core_event'  # Product_B 核心转化事件
...
def main():
    parser = argparse.ArgumentParser(
        description='Conversion Funnel Intelligence — Product_B 漏斗分析工具'
    )
    parser.add_argument('--af', required=True, help='AppsFlyer 事件 CSV 文件路径')
    parser.add_argument('--crm', required=True, help='CRM xlsx 文件路径')
    parser.add_argument('--output', default='product_b_funnel_result.xlsx', help='输出 Excel 路径')
    ...
    # ── Step 5：加载 CRM 数据 ───────────────
    print(f'[Step 5] 加载 CRM 数据：{args.crm}')
    try:
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = 'Product_B Funnel Analysis'
...
```bash
python3 product_b_funnel.py \
    --af product_b_in-app-events_20260501_20260531.csv \
    --crm crm_data.xlsx \
    --output product_b_funnel_result.xlsx
```
...
`product_b_funnel_result.xlsx`（带颜色标注）

| Channel | AF Events | CRM Conversions | Validity% | Classification | Factor | AF_CAP | Est.CRM | Action |
|---------|-----------|-----------------|-----------|----------------|--------|--------|---------|--------|
| channel_a | 15000 | 5200 | 34.67 | 高有效性 | 1.2 | 18000 | 6238 | 增加投放 |
| channel_b | 8000 | 2000 | 25.00 | 中有效性 | 1.0 | 8000 | 2000 | 维持不变 |
| channel_c | 5000 | 400 | 8.00 | 低有效性 | 0.5 | 2500 | 200 | 降低CAP |
...
- Product_B 是某地区主流小额信贷应用，核心转化为账户创建
...
- "Product_B 预算"、"Product_B 分析"、"Product_B 渠道"
- "CRM 数据"、"转化漏斗"
- "渠道有效性分析"、"UA 预算分配"
